async function loadAndDecodeSound(url, ctx) {
  ...
  function playSound(ctx, buffer, startTime, endTime) {
    ...
    if (activeSources.length >= MAX_SOURCES) {
        console.warn(`Too many sources (${activeSources.length}), stopping oldest ones`);
        const toStop = activeSources.splice(0, 10); // Arrêter les 10 plus anciennes
        toStop.forEach(source => {
            try {
                source.stop();
                source.disconnect();
            } catch(e) {
    ...
    bufferSource.onended = () => {
        ...
        console.log(`Source cleaned. Active sources: ${activeSources.length}`);
    };
}
